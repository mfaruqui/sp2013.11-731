...
def get_brev_penalty(found, gold):
    ...
    if len(found.split()) > len(gold.split()):
	    ...
	    return exp(1-1.*len(gold.split())/len(found.split()))
...
for line, ans in zip(open(trainFile,'r'), open(ansFile,'r')):
    ...
    h1ClusPrecList = [brevH1Clus*get_precision(set(get_ngrams(h1Clus,n)), set(get_ngrams(refClus,n))) for n in range(1,5)]
    h2ClusPrecList = [brevH2Clus*get_precision(set(get_ngrams(h2Clus,n)), set(get_ngrams(refClus,n))) for n in range(1,5)]
    
    (precH1, recallH1) = get_precision_recall_harmonic_mean(set(h1.split()), set(ref.split()))
    (precH2, recallH2) = get_precision_recall_harmonic_mean(set(h2.split()), set(ref.split()))
    ...
    X.append(h1PrecList + h1ClusPrecList + [recallH1] + h2PrecList + h2ClusPrecList + [precH2, recallH2])
    Y.append(int(ans))
